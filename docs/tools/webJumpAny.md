...
页面里另外带了「WebView 诊断」和「桥调用台」：在目标 App 的内置浏览器里打开时，可以枚举 App 注入到页面上的原生对象（JSBridge）、列出并调用它们暴露的方法，以及做一次带 Cookie 的直连测试。用于判断能不能借 App 的原生能力（发请求 / 取 token）绕开 CORS 和 SameSite 去调它自家的接口。诊断只读属性、不调用任何方法，报告里也不含数据，可以安全外发。
